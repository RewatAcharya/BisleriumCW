...
namespace Bislerium.WebApp.Controllers
{
    ...
    {
        [HttpPost]
        public async Task<IActionResult> Create(UpvoteComment vote, Guid blogId)
        {
            using (var httpClient = new HttpClient())
            {
                ...
                using (var response = await httpClient.PostAsync("https://localhost:7281/api/UpVoteComment/Reaction", content))
                {
            ...
            return RedirectToAction("Index", "Comment", new { id = blogId, userId = vote.LikedUser });
        }
    }
}
